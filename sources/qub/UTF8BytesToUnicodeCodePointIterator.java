 ...
 */
public class UTF8BytesToUnicodeCodePointIterator implements UnicodeCodePointIterator
{
    ...
    private void populateCurrentUnicodeCodePoint()
    {
        ...
        while (!done)
        {
            ...
            {
                this.foundByteOrderMark = (this.currentUnicodeCodePoint == 0xFEFF);

                if (!this.foundByteOrderMark || this.returnByteOrderMark)
                {
                    done = true;
                }
                else if (!this.bytes.next())
                {
                    done = true;
                    this.currentUnicodeCodePoint = null;
                }
    ...
    @Override
    public UTF8BytesToUnicodeCodePointIterator setReturnByteOrderMark(boolean returnByteOrderMark)
    {
