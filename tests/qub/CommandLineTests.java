    ...
    public static void test(final TestRunner runner)
    {
        runner.testGroup(CommandLine.class, () ->
        {
            runner.testGroup("create()", () ->
            {
                ...
                {
                    test.assertThrows(() -> CommandLine.create((String[])null), new PreConditionFailure("rawCommandLineArguments cannot be null."));
                });
                ...
                {
                    final CommandLine commandLine = CommandLine.create(new String[0]);
                    test.assertEqual(Iterable.create(), commandLine.getArgumentStrings());
                });

                runner.test("with non-empty String[]", (Test test) ->
                {
                    final CommandLine commandLine = CommandLine.create(new String[] { "a", "b", });
                    test.assertEqual(Iterable.create("a", "b"), commandLine.getArgumentStrings());
                });

                runner.test("with String argument list", (Test test) ->
                {
                    final CommandLine commandLine = CommandLine.create("a", "b", "c", "d");
            ...
            runner.testGroup("getValue()", () ->
            {
                ...
                {
                    final CommandLine commandLine = CommandLine.create(new String[] { "hello", "there" });
                    test.assertNull(commandLine.getValue(null));
                });
                ...
                {
                    final CommandLine commandLine = CommandLine.create(new String[] { "-name=value" });
                    test.assertNull(commandLine.getValue(null));
                });
            ...
            runner.testGroup("removeAt()", () ->
            {
                ...
                runner.test("with zero index with non-empty command line", (Test test) ->
                {
                    ...
                    test.assertEqual(CommandLine.create("b", "c"), commandLine);
                });
            });
            ...
            runner.testGroup("remove(String)", () ->
            {
                ...
                runner.test("with \"a\" when \"a\" exists", (Test test) ->
                {
                    ...
                    test.assertEqual(CommandLine.create("-c"), commandLine);
                });
            });
            ...
            runner.testGroup("equals()", () ->
            {
                ...
                runner.test("with equal", (Test test) ->
                {
                    ...
                    final CommandLine commandLine = CommandLine.create(commandLineStrings);
                    test.assertTrue(commandLine.equals((Object)CommandLine.create(commandLineStrings)));
                    test.assertTrue(commandLine.equals(CommandLine.create(commandLineStrings)));
                });

                runner.test("with different", (Test test) ->
                {
                    final CommandLine commandLine = CommandLine.create("-a=b", "-c");
                    test.assertFalse(commandLine.equals((Object)CommandLine.create(new String[] { "-d" })));
                    test.assertFalse(commandLine.equals(CommandLine.create("-d")));
                });
            });
            ...
            runner.testGroup("toString()", () ->
            {
                ...
                {
                    final CommandLine commandLine = CommandLine.create("a");
                    test.assertEqual("[a]", commandLine.toString());
                });

                runner.test("with two arguments", (Test test) ->
                {
                    final CommandLine commandLine = CommandLine.create("a", "b");
