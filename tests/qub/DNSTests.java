    ...
    public static void test(TestRunner runner, Function0<DNS> creator)
    {
        runner.testGroup(DNS.class, () ->
        {
            runner.testGroup("resolveHost(String)", () ->
            {
                ...
                {
                    final DNS dns = creator.run();
                    test.assertEqual(IPv4Address.parse("1.2.3.4"), dns.resolveHost("1.2.3.4").awaitError());
                });

                runner.test("with \"www.example.com\"", runner.skip(!runner.hasNetworkConnection().await()), (Test test) ->
                {
                    final DNS dns = creator.run();
                    test.assertEqual(IPv4Address.parse("93.184.216.34"), dns.resolveHost("www.example.com").awaitError());
                });

                runner.test("with \"spam.example.com\"", (Test test) ->
                {
                    final DNS dns = creator.run();
                    test.assertThrows(() -> dns.resolveHost("spam.example.com").awaitError(),
                ...
                {
                    final DNS dns = creator.run();
                    test.assertEqual(IPv4Address.parse("93.184.216.34"), dns.resolveHost("example.com").awaitError());
                });

                runner.test("with \"com\"", (Test test) ->
                {
                    final DNS dns = creator.run();
                    test.assertThrows(() -> dns.resolveHost("com").awaitError(),
