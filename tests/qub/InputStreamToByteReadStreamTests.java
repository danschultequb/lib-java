...
public interface InputStreamToByteReadStreamTests
{
    static void test(TestRunner runner)
    {
        ...
        {
            runner.testGroup("create(java.io.InputStream)", () ->
            {
                runner.test("with null InputStream", (Test test) ->
                {
                    test.assertThrows(() -> InputStreamToByteReadStream.create(null),
                        new PreConditionFailure("inputStream cannot be null."));
                });

                runner.test("with non-null InputStream", (Test test) ->
                {
                    final InputStreamToByteReadStream byteReadStream = InputStreamToByteReadStream.create(InputStreamToByteReadStreamTests.getInputStream(5));
            ...
            runner.testGroup("readByte()", () ->
            {
                ...
                runner.test("when exception is thrown", (Test test) ->
                {
                    ...
                    final InputStreamToByteReadStream byteReadStream = InputStreamToByteReadStreamTests.getByteReadStream(test, inputStream);
    ...
    {
        return InputStreamToByteReadStream.create(inputStream);
    }
}
